...
- Web Api должен предоставлять эндпоинты для выполнения действий с задачами:
  ...
  - POST /status - для изменения статуса задачи;
  - PUT - для редактирования задачи;
  - GET /:id - для получения конкретной задачи задачи по ее Id
  - GET - для получения списка всех задач
  - POST /filter - для получения списка задач, в теле запроса фильтр json для фильтрации задач по
    - Статусу;
    - Дате создания
...
- Реализован Web Api - 3 балла.
- Написаны юнит-тесты - 2 балл.
- Тесты адекватны и полностью покрывают функциональность - 2 балл.
